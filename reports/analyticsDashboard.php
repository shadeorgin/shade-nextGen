...
try {
    ...
    $categoryQuery = "SELECT
        CASE 
            WHEN t.appealId = 0 THEN 'Food'
            ELSE COALESCE(a.Cause, 'General')
        END as category,
        b.Type,
        COUNT(DISTINCT b.Id) as beneficiary_count
        FROM TblTxDetails t
        LEFT JOIN TblAppealInfo a ON t.appealId = a.Id
        LEFT JOIN TblBeneficiary b ON a.BeneficiaryId = b.Id
        WHERE YEAR(t.DateOfTx) = :year
        AND (a.id <> -1 OR t.appealId = 0)
        GROUP BY 
            CASE WHEN t.appealId = 0 THEN 'Food' ELSE COALESCE(a.Cause, 'General') END,
            b.Type";
